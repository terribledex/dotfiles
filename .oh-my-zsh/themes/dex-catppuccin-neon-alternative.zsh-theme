# Catppuccin Neon Theme для Oh My Zsh
# Автор: Custom theme
# Версия: 1.0
# Описание: Двухэтажный промпт в стиле Catppuccin с неоновыми акцентами
...
git_prompt_info_custom() {
    if git rev-parse --git-dir &> /dev/null; then
        ...
        echo " $(catppuccin_color mauve)━━$(catppuccin_reset) $(catppuccin_color sapphire)$(catppuccin_reset) $(catppuccin_color text)$branch_name$(catppuccin_reset) $(catppuccin_color $status_color)$git_status$(catppuccin_reset)"
    fi
}
...
build_prompt() {
    ...
    local current_path="$(catppuccin_color lavender)━━$(catppuccin_reset) $(catppuccin_color pink) PWD$(catppuccin_reset) $(catppuccin_color yellow)$(shrink_path)$(catppuccin_reset)"
    local git_info="$(git_prompt_info_custom)"
    
    # Левая часть промпта (первая строка)
    local left_prompt="${user_host}${current_path}${git_info}"
    
    # Правая часть промпта (время и время выполнения)
    local current_time="$(catppuccin_color teal)%D{%H:%M:%S}$(catppuccin_reset)"
    local cmd_time=""
    local duration=$(get_cmd_duration)
    if [[ -n $duration ]]; then
        cmd_time=" $(catppuccin_color blue)━━$(catppuccin_reset) $(catppuccin_color peach) EXEC$(catppuccin_reset) $(catppuccin_color subtext1)$duration$(catppuccin_reset)"
    fi
    
    local right_prompt="${cmd_time} $(catppuccin_color green)━━$(catppuccin_reset) $(catppuccin_color lavender) TIME$(catppuccin_reset) ${current_time}"
    ...
    # Убеждаемся, что количество пробелов не отрицательное
...
# Функция для обновления заголовка терминала
...
# Сообщение об успешной загрузке темы
if [[ -o interactive ]]; then
    echo "$(catppuccin_color green)✓$(catppuccin_reset) Catppuccin Neon тема загружена!"
    if ! command -v bc >/dev/null 2>&1; then
        echo "$(catppuccin_color yellow)⚠$(catppuccin_reset)  Для точного измерения времени выполнения установите 'bc': brew install bc"
    fi
fi\n'"${second_line}"'
}
